...
def check_tiktok(username, driver):
...
def main():
    ...
    if not os.path.isfile(chrome_path):
        print("❌ ملف chromedriver.exe غير موجود في مجلد السكريبت.")
        return

    chrome_options = Options()
    # chrome_options.add_argument("--headless")  # ← ممكن تشغله بدون متصفح ظاهر
    ...
    print("✅ تم تشغيل المتصفح بنجاح، جاري فحص حسابات TikTok...\n")
    ...
    for idx, line in enumerate(lines, 1):
        ...
        status = check_tiktok(username, driver)
        ...
        if status == "live":
            ...
            print(f"[{idx}] ⚠️ الحالة غير واضحة: {status}")
    ...
    print(f"✅ الحية (Live): {live_count}")
    print(f"❌ المحذوفة أو المقفولة (Suspended): {suspended_count}")
